...
use super::rpc_version::RpcVersion;
use super::RpcType;
...
pub struct ServerConfig {
    ...
    pub rpc_version_default: HashMap<RpcType, RpcVersion>,
...
pub async fn start_server(
    ...
    let local_addr = listener
        .local_addr()
        .context("Failed to retrieve local address after binding TCP listener")?;
    ...
    let make_service = hyper::service::make_service_fn(move |_| {
        let cfg = cfg.clone();
        let rpc_version_default_1 = rpc_version_default.clone();
        async move {
            let cfg = cfg.clone();
            ...
            Ok::<_, Infallible>(hyper::service::service_fn(move |req| {
                ...
                let metrics_layer =
                    RpcMiddlewareLayerMetrics::new(Metrics::new(metrics, transport_label));

                let rpc_version_default_2 = rpc_version_default_1.clone();
                let rpc_middleware = jsonrpsee::server::RpcServiceBuilder::new()
                    .layer_fn(move |service| {
                        RpcMiddlewareServiceVersion::new(
                            service,
                            path.clone(),
                            rpc_version_default_2.clone(),
                        )
                    })
...
pub(crate) fn host_filtering(
    ...
    if enabled {
        ...
        Some(
            jsonrpsee::server::middleware::http::HostFilterLayer::new(hosts)
                .expect("Valid hosts; qed"),
        )
    } else {
        None
    }
