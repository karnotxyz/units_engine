...
fn validate_version(version: &str) -> Result<(), syn::Error> {
    ...
    for part in parts {
        if part.is_empty() || !part.chars().all(|c| c.is_ascii_digit()) {
            return Err(syn::Error::new(
                Span::call_site(),
                "Each part of the version must be a non-empty number",
            ));
        }
    }
...
pub fn versioned_rpc(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let train_name_with_version =
        syn::Ident::new(&format!("{trait_name}{version}"), trait_name.span());
    ...
    let trait_with_version = syn::ItemTrait {
        attrs: vec![
            syn::parse_quote!(#[jsonrpsee::proc_macros::rpc(server, client, namespace = #namespace)]),
        ],
