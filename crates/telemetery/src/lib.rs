// Taken from https://github.com/madara-alliance/madara/blob/main/madara/crates/client/analytics/src/lib.rs
...
impl Telemetery {
    ...
    fn init_tracer_provider(&self) -> anyhow::Result<Tracer> {
        ...
        let otel_endpoint = self.collection_endpoint.clone().ok_or(anyhow::anyhow!(
            "OTEL endpoint is not set, not initializing otel providers."
        ))?;
    ...
    fn format_event(
        ...
        match (level, target) {
            (&Level::INFO, "rpc_calls") => {
                ...
                let status_style = if status == "200" {
